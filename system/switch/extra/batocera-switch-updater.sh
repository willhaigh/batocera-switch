         ...
         ####################################################################
            u=/userdata/system/switch/extra/batocera-switch-desktopupdater.sh
               rm "$u" 2>/dev/null
                  ...
                  echo "  wget -q --tries=10 --no-check-certificate --no-cache --no-cookies -O /tmp/.batocera-switch-updater.sh https://raw.githubusercontent.com/willhaigh/batocera-switch/main/system/switch/extra/batocera-switch-updater.sh" >> "$u"
                  ##echo "  curl -sSf https://raw.githubusercontent.com/willhaigh/batocera-switch/main/system/switch/extra/batocera-switch-updater.sh -o /tmp/.batocera-switch-updater.sh " >> "$u"
   ...
   yuzu_custom_version=$(cat /userdata/system/switch/CONFIG.txt | grep "USE_YUZU_VERSION" | head -n1 | sed 's, ,,g' | cut -d "=" -f2 | sed 's, ,,g' | tr -d '\0')
      if [[ "$yuzu_custom_version" != "auto" ]] && [[ "$yuzu_custom_version" != "Auto" ]] && [[ "$yuzu_custom_version" != "AUTO" ]] && [[ "$yuzu_custom_version" != "A" ]] && [[ "$yuzu_custom_version" != "a" ]] && [[ "$yuzu_custom_version" != "" ]]; then 
           ...
           if [[ "$yuzu_custom_version_date" != "" ]] && [[ "$yuzu_custom_version_hash" != "" ]]; then 
              link_yuzu=$(echo "https://github.com/yuzu-mirror/yuzu-downloads/raw/main/Mainline%20Build%20-%20537296095%20(2024-03-04)/yuzu-mainline-20240304-537296095.AppImage")
         fi
      fi
   ### yuzuEA
